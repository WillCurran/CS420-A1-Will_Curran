...
class Vertex:
    ...
    def getPrevWeight(self):
        "return the weight along parent edge"
        if(self.parent != None):
            parent_edge = self.getParentEdge()
            if(parent_edge != None):
                return parent_edge.weight
        return 0
...
class Graph:
    "An adjacency list representation of a undirected graph"
    def __init__(self):
        self.verts = []
        self.edges = []
